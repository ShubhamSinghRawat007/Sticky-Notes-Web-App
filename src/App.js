...
const App = () => {
  ...
  */
  ...
  const isInitialMount = useRef(true); // ✅ Used to prevent first-run save
  ...
  // Save notes to localStorage after initial mount
  useEffect(() => {
  ...
  const addNote = () => {
    ...
    if (!lastNote) {
      let randomX = Math.floor(
        Math.random() * (window.innerWidth - DEFAULT_NOTE_WIDTH)
      );
      let randomY = Math.floor(
        Math.random() * (window.innerHeight - DEFAULT_NOTE_HEIGHT)
      );
